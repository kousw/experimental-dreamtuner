...
class NormalBaeDetector:
    def __init__(self):
        ...
        modelpath = os.path.join(ckpt_path, "scannet.pt")
        if not os.path.exists(modelpath):
            from basicsr.utils.download_util import load_file_from_url
            load_file_from_url(remote_model_path, model_dir=ckpt_path)
    ...
    def __call__(self, input_image):
        ...
        is_numpy = False
        with torch.no_grad():
            if isinstance(image_normal, np.ndarray): 
                assert image_normal.ndim == 3
                is_numpy = True
            ...
            if is_numpy:
                normal = rearrange(normal[0], 'c h w -> h w c')
                normal = normal.cpu().numpy()
                normal = (normal * 255.0).clip(0, 255).astype(np.uint8)
